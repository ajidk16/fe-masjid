...
export const load = async ({ url, cookies }) => {
	...
	const donations = await api(`/references/halal-status?page=${page}&limit=${limit}`, {
		method: 'GET',
...
export const actions = {
	createDonation: async ({ request, cookies }) => {
		...
		const newDonation = await api('/references/halal-status', {
			method: 'POST',
			headers: getAuthHeader(cookies.get('auth_token')),
			body: JSON.stringify({
	...
	updateDonation: async ({ request, cookies }) => {
		...
		const updatedDonation = await api(`/references/halal-status/${donationData.id}`, {
			method: 'PUT',
			headers: getAuthHeader(cookies.get('auth_token')),
			body: JSON.stringify({
				code: donationData.code,
	...
	deleteDonation: async ({ request, cookies }) => {
		...
		const deletedDonation = await api(`/references/halal-status/${id}`, {
			method: 'DELETE',
			headers: getAuthHeader(cookies.get('auth_token'))
		});
